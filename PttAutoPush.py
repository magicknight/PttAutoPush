...
def post(board, push, index, tag) :
        ...
        while 1 : 
           ...
           time.sleep(2)
           content = telnet.read_very_eager().decode('big5','ignore')
           if( (u'特殊文章，無價格記錄' not in content) and (u'文章代碼(AID)' in content) ):          
              start = content.find(u'文章代碼(AID)')
              end = content.find(u'這一篇文章')
              contentdata = content[start:end]        
              start = contentdata.find('#')
              end = contentdata.find( '('+board )
              AID = contentdata[start:end].strip();
              if (AID not in list_AID ):
                 list_AID.append(AID)
                 telnet.write('\r\nX')   # "X" = 推文
                 time.sleep(2)              
                 content = telnet.read_very_eager().decode('big5','ignore')   
